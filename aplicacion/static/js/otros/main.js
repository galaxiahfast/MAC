...
/**
 * @galaxiahfast Carga todos los dispositivos existentes desde el servidor.
 * @galaxiahfast Los muestra en el mapa sin efectos especiales.
 */
async function CargarTodosLosDispositivos() {
    try {
        ...
        /* @galaxiahfast Validar respuesta del servidor (usando 'estado' como devuelve el backend) */
        if (data.estado === 'exito') {
            ...
            console.error("❌ Error al cargar dispositivos:", data.mensaje || data.message);
        }
    } catch (error) {
...
/* ==========================================================================
   GESTIÓN DE APARTADOS: CONTROL DE VISIBILIDAD
   ========================================================================== */
...
/* ==========================================================================
   LÓGICA DE ELIMINACIÓN DE APARTADOS
   ========================================================================== */
    ...
    if (btnDeleteTool) {
        btnDeleteTool.onclick = async (e) => {
            ...
            if (isVisible && isEliminarVisible) {
                ...
                /* @galaxiahfast Usar caché unificado window.cacheApartados */
                if (window.cacheApartados) {
                    UI_RenderizarListaEliminacion(window.cacheApartados);
                } else {
...
/* ==========================================================================
   PROCESO DE GUARDADO DE APARTADOS (AGREGAR)
   ========================================================================== */
...
/* ==========================================================================
   FUNCIONES DE RESETEO Y UTILIDADES
   ========================================================================== */

function UI_CerrarTodosLosModales() {
    /* @galaxiahfast Cerrar modal de configuración de parámetros */
    ...
    /* @galaxiahfast Cerrar modal de agregar punto */
...
/* ==========================================================================
   RENDERIZADO DE LISTA PARA ELIMINACIÓN
   ========================================================================== */
...
async function ObtenerApartadosActivos(forzarRefresh = false) {
    /* @galaxiahfast Usar caché unificado window.cacheApartados */
    if (!forzarRefresh && window.cacheApartados) {
        apartadosActivosFormulario = window.cacheApartados;
...
function ajustarPaddingSegunScroll() {
    ...
    grupos.forEach(grupo => {
        if (tieneScroll) {
            grupo.style.paddingRight = '25px';
        } else {
            grupo.style.paddingRight = '10px';
        }
    });
}
...
/* ==========================================================================
   FUNCIONES DEL MAPA (PUNTOS VISUALES)
   ========================================================================== */

/**
 * @galaxiahfast Limpia un texto para usarlo como ID de DOM.
 */
...
/**
 * @galaxiahfast Agrega un punto visual al mapa.
 */
function UI_AgregarPuntoAlMapa(dispositivo, conEfecto = true) {
    ...
    if (typeof x === 'string' && !x.includes('%') && !x.includes('px')) x = x + 'px';
    if (typeof y === 'number') y = y + 'px';
    if (typeof y === 'string' && !y.includes('%') && !y.includes('px')) y = y + 'px';
...
/**
 * @galaxiahfast Carga los apartados desde el servidor y actualiza caché.
 */
async function cargarApartados(forzarActualizacion = false) {
    ...
    try {
        ...
        mostrarNotificacion("No se pudieron cargar los parámetros", "error");
        return [];
    }
}