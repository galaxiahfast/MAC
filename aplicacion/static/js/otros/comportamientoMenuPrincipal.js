function iniciarComportamientoMenuLateralPrincipal() {
    ...
    const contenedorBarraHerramientas = document.querySelector(
        '.contenedor-barra-herramientas-principal'
    );

    const contenedorInternoBarraHerramientas = document.querySelector(
        '.contenedor-interno-barra-herramientas'
    );

    const botonAlternarExpansionMenu = document.getElementById(
        'botonAlternarExpansionMenuLateralPrincipal'
    );

    if (
        !contenedorBarraHerramientas ||
        !contenedorInternoBarraHerramientas ||
        !botonAlternarExpansionMenu
    ) {
        console.error(
            'No se pudo inicializar el menú lateral principal.'
        );
    ...
    function calcularAnchoAutomaticoMenuExpandido() {
        ...
        const botonesMenu =
            contenedorInternoBarraHerramientas.querySelectorAll(
                selectorBotonesMenu
            );
    ...
    function agregarTextoExpandidoBoton(boton, texto) {
        ...
        if (
            boton.querySelector('.texto-expandido-boton-menu')
        ) {
            return;
        }
    ...
    function agregarTituloSeparadorMenu(
        separador,
        titulo
    ) {
        ...
        if (
            separador.querySelector(
                '.contenedor-titulo-separador-menu'
            )
        ) {
            return;
        }
    ...
    function expandir() {
        ...
        const ancho =
            calcularAnchoAutomaticoMenuExpandido();

        contenedorBarraHerramientas.classList.add(
            'menu-lateral-expandido'
        );

        contenedorBarraHerramientas.style.setProperty(
            '--ancho-expandido-menu-lateral',
            `${ancho}px`
        );
        ...
        contenedorInternoBarraHerramientas
            ...
            .forEach(boton => {
                ...
                agregarTextoExpandidoBoton(
                    boton,
                    textosBotonesExpandibles[boton.id]
                );
        ...
        contenedorInternoBarraHerramientas
            .querySelectorAll(
                '.contenedor-separador-seccion-menu-lateral'
            )
            .forEach(separador => {

                agregarTituloSeparadorMenu(
                    separador,
                    titulosSeparadoresMenu[
                        separador.dataset.seccionMenu
                    ]
                );
    ...
    function colapsar() {
        ...
        contenedorBarraHerramientas.classList.remove(
            'menu-lateral-expandido'
        );

        contenedorBarraHerramientas.style.removeProperty(
            '--ancho-expandido-menu-lateral'
        );

        document
            .querySelectorAll(
                '.texto-expandido-boton-menu'
            )
    ...
    botonAlternarExpansionMenu.addEventListener(
        'click',
        evento => {

            evento.stopPropagation();

            menuLateralExpandido
                ? colapsar()
                : expandir();
...
document.addEventListener(
    'DOMContentLoaded',
    iniciarComportamientoMenuLateralPrincipal
);